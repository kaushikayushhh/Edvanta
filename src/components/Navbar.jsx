...
import logo from "../assets/edvantalogo.png";
          ...
          {navLinks.map((link, index) => (
            <motion.a 
              ...
              className={`text-sm font-medium relative after:absolute after:bottom-0 after:left-0 after:h-0.5 after:w-0 hover:after:w-full after:bg-red-700 after:transition-all
                ${activeLink === link.href ? 'text-black-600 after:w-full  ' : 'text-gray-600 hover:text-gray-900'}`}
            >
              {link.label}
            </motion.a>
          ))}
        ...
        >
          ...
          >
            {navLinks.map((link, index) => (
              <motion.a
                ...
                className={`block text-sm font-medium py-2
                  ${activeLink === link.href ? 'text-black-600' : 'text-gray-600 hover:text-gray-900'}`}
              >
                {link.label}
              </motion.a>
            ))}
            <motion.button 
              ...
              whileTap={{ scale: 0.98 }}
              className="w-full bg-red-600 text-white px-6 py-2.5 rounded-lg hover:bg-red-700 text-sm font-medium transition-all hover:shadow-lg hover:shadow-blue-100"
            >
              Get in touch
            </motion.button>
          </motion.div>
